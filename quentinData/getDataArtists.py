...
output_file = "src\data\quentin_data_artists.csv"
...
filtered_data = [item for item in data["results"]["bindings"] if "nbMembers" in item and (int(item["nbMembers"]["value"]) == 0 or int(item["nbMembers"]["value"]) > 1)]
...
for item in filtered_data:
    ...
    nb_members = "Yes" if int(item["nbMembers"]["value"]) > 1 else "No"
...
with open(output_file, 'w', newline='', encoding='utf-8') as csv_file:
    ...
    csv_writer.writerow(["Nom de l'artiste", "Date d'anniversaire", "Titre", "Genre", "Date de sortie", "Groupe"])
