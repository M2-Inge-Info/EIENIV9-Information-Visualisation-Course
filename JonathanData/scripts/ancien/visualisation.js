...
const displayEquipementDetails = (genre) => {
    d3.json("data/equipment_counts.json").then(function(data) {
        ...
        if (data[genre]) {
            ...
            let svg = equipementDetailsDiv.append("svg").attr("width", 1000).attr("height", 600);

            let equipementData = Object.entries(data[genre]);

            let xScale = d3.scaleBand()
                .domain(equipementData.map(d => d[0]))
                .range([0, 500])
                .padding(0.1);

            let yScale = d3.scaleLinear()
                .domain([0, d3.max(equipementData, d => d[1])])
                .nice()
                .range([500, 0]);
            ...
            equipementData.forEach((d, i) => {
                ...
                legendItem.append("rect")
                    .attr("width", 18)
                    .attr("height", 18)
                    .style("fill", "steelblue");  // Assurez-vous d'utiliser la même couleur que vos barres
